...
if [[ -n "${downstream_run_id}" && "${run_status}" == "completed" ]]; then
  if [[ "${run_conclusion}" == "success" ]]; then
    echo "Downstream workflow run ${downstream_run_id} already completed successfully."
    exit 0
  fi
  echo "Previous downstream workflow run ${downstream_run_id} completed with ${run_conclusion}; dispatching a new attempt."
  previous_run_id="${downstream_run_id}"
  downstream_run_id=""
elif [[ -n "${downstream_run_id}" ]]; then
  echo "Resuming downstream workflow run ${downstream_run_id}, currently ${run_status}."
else
  previous_run_id=""
fi

if [[ -z "${downstream_run_id}" ]]; then
  if ! timeout 30s gh workflow run "${TARGET_WORKFLOW}" \
    --repo "${TARGET_REPOSITORY}" \
    --ref "${TARGET_REF}" \
    ...
    echo "Could not confirm downstream workflow dispatch. Checking for an accepted run." >&2
  fi
  ...
  while ((SECONDS < discovery_deadline)); do
    ...
    if ((list_exit == 0)); then
      ...
      echo "Could not list downstream runs during discovery (exit ${list_exit})." >&2
    fi
    sleep 2
  done

  if [[ -z "${downstream_run_id}" || "${downstream_run_id}" == "${previous_run_id}" ]]; then
    echo "Could not find the dispatched downstream workflow run." >&2
    exit 1
  fi
fi
...
for watch_attempt in {1..3}; do
  ...
  if ! run_result="$(view_run "${downstream_run_id}")"; then
    echo "Could not determine the state of downstream workflow run ${downstream_run_id}. Rerun synchronization to resume monitoring." >&2
    exit 1
  fi
  ...
  if [[ "${run_status}" == "completed" ]]; then
    ...
    echo "Downstream workflow run ${downstream_run_id} completed with ${run_conclusion} (watch exit ${watch_exit})." >&2
    exit 1
  fi
  ...
  if ((watch_attempt == 3)); then
    echo "Monitoring failed after three attempts (exit ${watch_exit}). Rerun synchronization to resume run ${downstream_run_id}." >&2
    exit 1
  fi
...
if [[ "${CANCEL_ON_TIMEOUT:-true}" == false ]]; then
  echo "Leaving downstream run ${downstream_run_id} active after the monitoring timeout. Rerun synchronization to resume monitoring." >&2
  exit 1
fi

echo "Downstream workflow run ${downstream_run_id} exceeded 45 minutes while ${run_status}; cancelling it." >&2
cancel_exit=0
timeout 30s gh run cancel "${downstream_run_id}" \
  --repo "${TARGET_REPOSITORY}" || cancel_exit=$?
...
if [[ "${run_conclusion}" == "success" ]]; then
  echo "Downstream workflow run ${downstream_run_id} completed successfully during cancellation."
  exit 0
fi
if [[ "${run_status}" != "completed" ]]; then
  echo "Downstream workflow run ${downstream_run_id} did not stop after cancellation." >&2
  exit 1
fi
echo "Downstream workflow run ${downstream_run_id} stopped with ${run_conclusion}." >&2
exit 1
